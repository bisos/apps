...
(define (script-fu-idPhoto inImage inLayer outDPI outMode outWD outHT inMode inWD inHT wLeft
  ...
  (let* (
    ...
    (gimp-rect-select theImage selNX selNY selWD selHT CHANNEL-OP-REPLACE 0 0)
		...
		(while (< cntR row)
			...
			(set! sX sW)
			(while (< cntC col)
				(gimp-rect-select outImage sX sY frameWidth frameHeight CHANNEL-OP-REPLACE 0 0)
				(gimp-edit-fill frameLayer FOREGROUND-FILL)
				...
				(gimp-image-add-layer outImage newLayer 0)
				(set! offX (+ sX wLeft))
				(set! offY (+ sY wTop))
...
(script-fu-register "script-fu-idPhoto"
		    ...
		    SF-ADJUSTMENT _"DPI"  '(300 10 1000 10 100 0 0)
		    
		    SF-OPTION     _"Print paper size"    '("4x6 in" "3.5x5 in" "Custom")
		    SF-ADJUSTMENT _"Width(pixels)" '(1800 100 6000 10 100 0 0)
		    SF-ADJUSTMENT _"Height(pixels)" '(1200 100 6000 10 100 0 0)
		    
		    SF-OPTION     _"idPhoto size"   '("2 in" "1 in" "2 in(US)" "Custom")
		    SF-ADJUSTMENT _"idPhoto width(cm)" '(3.50 1.00 10.00 0.01 0.50 1 0)
		    SF-ADJUSTMENT _"idPhoto height(cm)" '(4.50 1.00 10.00 0.01 0.50 1 0)
		    
		    SF-ADJUSTMENT _"Border left(pixels)" '(8 0 200 1 10 0 0)
		    SF-ADJUSTMENT _"Border top(pixels)" '(8 0 200 1 10 0 0)
		    SF-ADJUSTMENT _"Border right(pixels)" '(8 0 200 1 10 0 0)
		    SF-ADJUSTMENT _"Border bottom(pixels)" '(8 0 200 1 10 0 0)
		    
		    SF-TOGGLE	_"Display border"	FALSE
